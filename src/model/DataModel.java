...
public class DataModel
{
	 ...
	 * Lowest year
	 */
	private double lowestYear;

	/**
	 * Highest year
	 */
	private double highestYear;
	...
	public DataModel(final String [] filenames) throws FileNotFoundException, InvalidFileFormatException
	{
		...
		switch(indicatorType)
		{
			...
			case SCHOOL_ENROLLMENT_SECONDARY:
				xAxisName = "Year";
				yAxisName = IndicatorType.SCHOOL_ENROLLMENT.getLabel();
				break;
			default:
	...
	/**
	 * Void method that parses the CSV File
	 * Creates different Indicators for the different types of values
	 */
	private void parseCSVFile(final String filename) throws FileNotFoundException, InvalidFileFormatException
	{
		...
		for (int countryIndex = 0; countryIndex < model.length; countryIndex++)
		{
			...
			lowestYear = Double.parseDouble(String.valueOf(yearLabels[0]));
			highestYear = Double.parseDouble(String.valueOf(yearLabels[yearLabels.length - 1]));
    ...
    public double getLowestYear() {
		return lowestYear;
    }

	public double getHighestYear() {
		return highestYear;
	}
}
